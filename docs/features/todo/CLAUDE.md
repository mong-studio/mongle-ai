...
1. **태그 어휘 (Tagger)** — 자유 형식 vs 사전 정의 enum. enum이라면 캐릭터 생성의 성격 키워드처럼 목록 확정 필요.
2. **멀티턴 세션 저장소** — Redis vs DB. TTL은? (대화 종료/확정 시 정리 정책)
3. **멀티턴 "수정 요청 시 회귀"의 범위** — 사용자가 플랜의 일부만 수정 요청해도 Planner부터 다시 도는지, Plan Generator만 부분 재실행인지.
4. **싱글턴의 시간(time_hint) 처리** — "오전", "저녁" 같은 시간 힌트를 TODO 정렬에만 쓰는지, 캘린더 이벤트 시작 시각으로 변환하는지.
5. **Plan Generator 응답 길이 초과 시 정책** — 재생성 vs 자동 잘라내기 vs 사용자에게 알림.
6. **멀티턴 입력 "한국어 위주" 검증 방법** — 언어 감지 라이브러리 사용 여부. 영어 섞인 입력은 허용/차단/경고 중 어느 것?
7. **퀘스트 분배 카운트 리셋 시점** — 자정 기준(서버 시각)? 사용자 로컬 자정?
