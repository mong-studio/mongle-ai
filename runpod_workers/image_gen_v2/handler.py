"""RunPod Serverless handler for the v2 Mongle image pipelines.
...
LOGGER = logging.getLogger("image_gen_v2")
...
def process_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "image_character":
        from total.image_pipeline2.handler import process_job as process_image
        ...
        from total.text_pipeline.handler import process_job as process_text

        persona = job_input.get("persona") or job_input.get("prompt")
        result = process_text({"input": {**job_input, "persona": persona}})
    else:
        from total.feed_pipeline2.handler import process_job as process_feed
...
def handler(job: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        LOGGER.error("image_gen_v2 failed: %s\n%s", exc, traceback.format_exc())
        return {"status": "failed", "error": "image_gen_v2 generation failed", "code": "generation_failed"}
