...
from adapters.character_creation.runpod_llm import RunPodQwenLLM
from agents.character_creation.exceptions import LLMFailedError

ENDPOINT = "https://api.runpod.ai/v2/test-ep"
MESSAGES = [{"role": "user", "content": "안녕"}]
_PATCH = "adapters.character_creation.runpod_llm.httpx.AsyncClient"


def _llm(**kw) -> RunPodQwenLLM:
    defaults = dict(endpoint_url=ENDPOINT, api_key="rp-key", adapter="character", poll_interval=0.0, poll_timeout=5.0)
    defaults.update(kw)
...
async def test_completed_returns_text() -> None:
    client = _mock_client(statuses=[{"status": "COMPLETED", "output": {"text": "페르소나"}}])
    with patch(_PATCH, return_value=client):
        result = await _llm()._complete_raw(messages=MESSAGES)
    assert result == "페르소나"
...
async def test_terminal_status_raises(status: str) -> None:
    ...
    with patch(_PATCH, return_value=client):
        with pytest.raises(LLMFailedError):
            await _llm()._complete_raw(messages=MESSAGES)
...
async def test_timeout_raises() -> None:
    ...
    with patch(_PATCH, return_value=client):
        with pytest.raises(LLMFailedError, match="timed out"):
            await _llm(poll_timeout=0.0)._complete_raw(messages=MESSAGES)
...
async def test_payload_includes_adapter() -> None:
    ...
    with patch(_PATCH, return_value=client):
        await _llm(adapter="character")._complete_raw(messages=MESSAGES)
    sent = client.post.call_args.kwargs["json"]
    assert sent["input"]["adapter"] == "character"
