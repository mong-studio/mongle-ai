...
from adapters.quest_generation.runpod_llm import RunPodQwenLLM
...
async def test_completed_returns_text() -> None:
    client = _mock_client(statuses=[{"status": "COMPLETED", "output": {"text": "퀘스트"}}])
...
async def test_payload_includes_adapter() -> None:
    ...
    with patch(_PATCH, return_value=client):
        await _llm(adapter="planner")._complete_raw(messages=MESSAGES)
