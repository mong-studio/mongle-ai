...
async def test_llm_caption_node_sets_raw_caption_and_routes_to_builder():
    fake_llm = FakeLLM(response="청소 완료! ✨")
    ports = make_ports(llm=fake_llm)
    state = make_state(caption_prompt="당신은 몽글이입니다...")
...
async def test_llm_caption_node_passes_full_prompt_to_llm():
    fake_llm = FakeLLM()
    ports = make_ports(llm=fake_llm)
    state = make_state(caption_prompt="테스트 프롬프트")
    config = {"configurable": {"ports": ports}}
...
async def test_llm_caption_node_wraps_port_error_as_caption_generation_error():
    ...
    state = make_state(caption_prompt="프롬프트")
    config = {"configurable": {"ports": ports}}
