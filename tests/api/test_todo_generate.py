...
def test_generate_compat_alias_returns_done_envelope(api_client):
    """Django 내부 호출용 /todo/generate alias도 같은 후보 결과를 반환한다."""
    api_client.app.dependency_overrides[get_todo_generate_ports] = _override
    body = {"user_id": "u1", "prompt": "내일 장보기", "today": "2026-06-04"}
    resp = api_client.post("/todo/generate", json=body, headers=AUTH)
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "done"
    assert data["result"]["todos"][0]["title"] == "장보기"
...
def test_generate_requires_api_key(api_client):
    ...
    assert api_client.post("/todo/generate", json=body).status_code == 401
