...
# 시험종류별 도메인 전략 힌트(LLM 프롬프트·폴백 요약에 주입). 일반적인 '기출 회독'을
# 넘어 각 시험의 실제 구조(과목·파트·합격기준)에 맞는 공략을 유도한다.
EXAM_STRATEGY: dict[str, str] = {
    "정보처리기사_필기": "5과목 기출 회독과 과락(40점) 방지를 우선하고 약점 과목 오답을 반복.",
    "토익": "LC(Part1~4)·RC(Part5~7)를 나눠, 약점 파트 중심으로 Part5 문법·Part7 독해 속도·Part3·4 쉐도잉/받아쓰기와 빈출 단어를 병행하고 ETS 공식 모의고사로 시간 배분 점검.",
    "한국사능력검정시험": "시대별 흐름과 빈출 사료·인물 중심으로 기출 회독, 등급 컷에 맞춰 약점 시대 보완.",
    "SQLD": "1과목 데이터 모델링 개념과 2과목 SQL 활용 문제 풀이를 반복, 빈출 SQL 구문 정리.",
    "컴활1급": "필기 이론과 실기 함수·매크로 실습을 분리해 실기 기출 반복.",
    "컴활2급": "필기 핵심 이론과 실기 스프레드시트·함수 실습을 기출 중심으로 반복.",
}
...
def _fallback_plan(seed: dict, today: date) -> PlanOutput:
    ...
    strategy = EXAM_STRATEGY.get(
        seed["exam_type"], "기출 회독으로 출제 감 잡고, 약점 영역을 집중 보완한 뒤 모의고사로 점검하세요."
    )
    summary = (
...
def build_exam_prompt(seed: dict, *, today: date, exemplars: list[dict]) -> str:
    ...
    for ex in exemplars:
        shots += f"\n[예시 플랜]\n{ex}\n"
    strategy = EXAM_STRATEGY.get(seed["exam_type"], "")
    strategy_line = f"이 시험 공략 힌트: {strategy}\n" if strategy else ""
    return (
...
def main() -> None:
    ...
    # 로컬 합성 모델 클라이언트는 --use-llm 일 때만 필요. 무거운/선택적 의존성을
    # 모듈 임포트에 묶지 않도록 사용 시점에 지연 임포트한다.
    client = None
    if args.use_llm:
