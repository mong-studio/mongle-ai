...
- project는 goal, success_criteria, horizon, available_time이 모두 있어야 충분하다.
...
- 딱딱한 설문 문장처럼 쓰지 않는다.
- 이미 들은 내용은 다시 묻지 않는다.
- 서로 관련된 핵심 조건은 한 문장에 2~4개까지 묶어 물을 수 있다.
...
- plan_kind=event 이면 마지막 날에 실제 경기/대회 출전 일정을 둔다.
...
- 실제 목표일이 30일 이후면 상세 일정은 첫 30일만 다루고 이후 월별 진행 방향을 summary_text 에 덧붙인다.
"""
